		...
		<div className="border-t border-gray-300 flex pt-8 xl:block xl:px-4">
			<div className="mr-12 flex-2 xl:mb-10">
				...
				<div>
					<StarRatingProgress
						rating={5}
						percentage={shoeRatings.filter((rating) => rating.ratingNum === 5).length / shoeRatings.length}
					/>
					<StarRatingProgress
						rating={4}
						percentage={shoeRatings.filter((rating) => rating.ratingNum === 4).length / shoeRatings.length}
					/>
					<StarRatingProgress
						rating={3}
						percentage={shoeRatings.filter((rating) => rating.ratingNum === 3).length / shoeRatings.length}
					/>
					<StarRatingProgress
						rating={2}
						percentage={shoeRatings.filter((rating) => rating.ratingNum === 2).length / shoeRatings.length}
					/>
					<StarRatingProgress
						rating={1}
						percentage={shoeRatings.filter((rating) => rating.ratingNum === 1).length / shoeRatings.length}
					/>
				</div>
